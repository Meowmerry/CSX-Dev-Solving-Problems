...
function makeHistory(limit) {
    // declare a variable history assign to empty array
    const history = [];
    // create a return function inter and accept string as arguments
    return (args) => {
        // check if args is equal to undo 
        if (args === 'undo') {
            // check if history length is equal to 0 return nothing to undo
            if (history.length === 0) {
                return 'nothing to undo';
                // otherwise, history pop out and join with string 'undone'
            } else {
                return history.pop() + ' undone';
            }
            // otherwise do something 
        } else {
            // if history length is equal to limit
            if (history.length === limit) {
                // shift out of the first history
                history.shift();
            }
            // fill the history by pushing args
            history.push(args);
            // return args join with done
            return args + ' done';
        }
    };
}
